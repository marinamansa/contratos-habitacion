// ── Plantilla base (cargada desde Supabase) ──────────
const SUPABASE_URL_CT = 'https://kyrjqbautyklefhqzqbo.supabase.co';
const SUPABASE_KEY_CT = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imt5cmpxYmF1dHlrbGVmaHF6cWJvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODExMDgwNjUsImV4cCI6MjA5NjY4NDA2NX0.EXBSK0ZLJUsWt1iQ8btA1FkQE_aPT_MFkdgEhkP4Gh0';
let _plantillaClausulas = {};

async function cargarPlantilla(){
  try {
    const res = await fetch(`${SUPABASE_URL_CT}/rest/v1/plantilla?id=eq.base`, {
      headers:{'apikey':SUPABASE_KEY_CT,'Authorization':`Bearer ${SUPABASE_KEY_CT}`}
    });
    const data = await res.json();
    _plantillaClausulas = data[0]?.clausulas || {};
  } catch(e){ _plantillaClausulas = {}; }
}

function getPlantillaTexto(key){
  return _plantillaClausulas[key] || null;
}
...
// ── applyEdiciones (per-contract edits from index.html) ──
...
<p>SEGUNDO.- Que la parte arrendataria manifiesta no encontrarse, ni ella ni su unidad familiar, en situación de vulnerabilidad económica y/o social a la fecha de la firma del presente documento.</p>
<p>TERCERO.- Que el arrendador ha convenido con el arrendatario el arrendamiento de la habitación amueblada número ${d.hnum}, cuya superficie y composición son perfectamente conocidas por el arrendatario. El arrendamiento de dicha habitación dará derecho al uso común con el resto de arrendatarios de las zonas comunes. Se adjunta inventario del mobiliario existente.</p>

<h2>CLÁUSULAS</h2>

<h2>PRIMERA.- OBJETO Y DESTINO</h2>
${cl('primera',`<p>Por medio del presente contrato, la parte arrendadora arrienda a la parte arrendataria la habitación número ${d.hnum} descrita en este contrato, quien la acepta en las condiciones pactadas en este documento.</p>
...
${cl('segunda',`<p>Las partes acuerdan expresamente la renuncia del arrendatario a impedir que el arrendador pueda acceder a las zonas comunes de la vivienda y a las habitaciones que circunstancialmente no se encuentren arrendadas. La violación de este derecho será considerada causa de resolución del contrato.</p>`)}
...
${cl('cuarta',`<p>El arrendatario abonará al arrendador, en concepto de renta, la cantidad de ${numLetras(d.renta)} (${d.renta}) euros mensuales, debiendo hacerlo de forma anticipada en el momento de su incorporación, mediante ingreso o transferencia bancaria en el número de cuenta ${d.aiban}.</p>
...
${cl('quinta',`<p>Los gastos generales, tales como la comunidad de propietarios, IBI, etc., serán asumidos por la parte arrendadora.</p>`)}

<h2>SEXTA.- SUMINISTROS</h2>
${cl('sexta',`<p>Se incluyen en la renta mensual pactada los siguientes suministros: internet, agua, recogida de basuras y electricidad, hasta un máximo conjunto de ${d.lsum} € por mes.</p>
...
${cl('septima',`<p>Las partes se someten al régimen establecido en el código civil.</p>`)}

<h2>OCTAVA.- NORMAS DE CONVIVENCIA</h2>
${cl('octava',`<p>La parte arrendataria se someterá durante toda la vigencia del contrato a las normas de la comunidad de propietarios. Se prohíbe expresamente la estancia de cualquier tipo de animal en el inmueble, salvo autorización escrita del arrendador.</p>
...
${cl('novena',`<p>La parte arrendadora será responsable de las reparaciones derivadas del uso normal y desgaste natural. El arrendatario se compromete a utilizar correctamente el mobiliario y electrodomésticos, asumiendo el coste de reparación o sustitución en caso de uso indebido o negligencia. Toda incidencia deberá comunicarse en un plazo máximo de 48 horas desde su detección.</p>`)}

<h2>DÉCIMA.- CONSUMIBLES Y UTENSILIOS DE LIMPIEZA</h2>
${cl('decima',`<p>La parte arrendadora proporcionará de forma inicial y de cortesía: detergente para lavadora, detergente para lavavajillas, jabón de manos, papel higiénico, bolsas de basura, escoba y fregona. Una vez agotados, será responsabilidad de los arrendatarios reponerlos.</p>`)}

<h2>DÉCIMO PRIMERA.- CESIÓN Y SUBARRIENDO</h2>
${cl('decimoprimera',`<p>El inmueble no se podrá ceder ni subarrendar de forma parcial ni total. El incumplimiento será causa de resolución inmediata del contrato.</p>`)}

<h2>DÉCIMO SEGUNDA.- DERECHO DE ADQUISICIÓN PREFERENTE</h2>
${cl('decimosegunda',`<p>La parte arrendataria renuncia expresamente al derecho de adquisición preferente tanto de la habitación como de la vivienda en la que se ubica.</p>`)}

<h2>DÉCIMO TERCERA.- FIANZA Y OTRAS GARANTÍAS ADICIONALES</h2>
${cl('decimotercera',`<p>La parte arrendataria entrega en este acto a la parte arrendadora un total de ${d.fianza} euros en concepto de FIANZA, equivalente a una mensualidad de renta, para garantizar el cumplimiento de sus obligaciones.</p>
...
${cl('decimocuarta', conAvalista
  ? `<p>D. ${d.av_nombre}, con DNI ${d.av_dni}, se constituye en avalista solidario de la parte arrendataria, respondiendo de todas las obligaciones dinerarias derivadas del presente contrato de forma solidaria con el arrendatario.</p>`
  : `<p>No aplica en el presente contrato.</p>`)}

<h2>DÉCIMO QUINTA.- DOMICILIO A EFECTO DE NOTIFICACIONES</h2>
${cl('decimoquinta',`<p>Las partes acuerdan que serán válidas las notificaciones enviadas a la parte arrendadora en ${d.adom}, y a la parte arrendataria en la habitación arrendada.</p>
...
${cl('decimosexta',`<p>El presente contrato es un contrato de arrendamiento de habitación no sometido a la Ley de Arrendamientos Urbanos sino al Código Civil.</p>`)}

<h2>DÉCIMO SÉPTIMA.- SOLUCIÓN DE CONFLICTOS</h2>
${cl('decimoseptima',`<p>Apartado 1.- Seguro de impago: ${conSeguro ? 'Las partes acuerdan la contratación de un seguro de impago de renta, cuyos términos y condiciones se adjuntan como anexo al presente contrato.' : 'No se aplica en este caso.'}</p>
